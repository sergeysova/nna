import { Int64, UInt64 } from "./arith";
...
describe("binaryArithTest", function() {
  ...
  it("testCompare", function() {
    var a = new UInt64(1234, 5678);
    var b = new UInt64(1234, 5678);
   ...
   */
  it("testAdd", function() {
    var a = new UInt64(/* lo = */ 0x89abcdef, /* hi = */ 0x01234567);
    var b = new UInt64(/* lo = */ 0xff52ab91, /* hi = */ 0x92fa2123);
    // Addition with carry.
    var c = a.add(b);
  ...
  it("testSub", function() {
    ...
    for (var i = 0; i < kLength; i++) {
      for (var j = 0; j < kLength; j++) {
        var a = new UInt64(loValues[i], hiValues[j]);
        var b = new UInt64(loValues[j], hiValues[i]);
        var c = a.add(b).sub(b);
  ...
  it("testMul", function() {
    ...
    for (var i = 0; i < testData.length; i++) {
      var a = new UInt64(testData[i][0], testData[i][1]);
  ...
  it("testStrings", function() {
    const testData: [number, number, string][] = [
    ...
    for (var i = 0; i < testData.length; i++) {
      var a = new UInt64(testData[i][0], testData[i][1]);
      var roundtrip = UInt64.fromString(a.toString()) as UInt64;
      expect(roundtrip).not.toEqual(null);
  ...
  it("testSignedInt64", function() {
    ...
    for (var i = 0; i < testStrings.length; i++) {
      var roundtrip = (Int64.fromString(testStrings[i]) as Int64).toString();
